#!/usr/bin/env python3
"""
Bus Digital Controller - Interfaz Tkinter
Raspberry Pi 3 B+ con pantalla táctil ARD-510
"""
...
LED_CONFIG = [
    {"pin": 5,  "nombre": "BIT 7", "color": "#ff4444", "color_dim": "#4a1111"},
    {"pin": 6,  "nombre": "BIT 6", "color": "#ff8844", "color_dim": "#4a2811"},
    {"pin": 13, "nombre": "BIT 5", "color": "#ffff44", "color_dim": "#4a4a11"},
    {"pin": 19, "nombre": "BIT 4", "color": "#44ff44", "color_dim": "#114a11"},
    {"pin": 26, "nombre": "BIT 3", "color": "#4488ff", "color_dim": "#112844"},
    {"pin": 12, "nombre": "BIT 2", "color": "#aa44ff", "color_dim": "#2a114a"},
    {"pin": 16, "nombre": "BIT 1", "color": "#ff44aa", "color_dim": "#4a1130"},
    {"pin": 20, "nombre": "BIT 0", "color": "#44ffff", "color_dim": "#114a4a"},
]
...
class BusControlApp:
    def __init__(self, root):
        ...
        # Fuentes
        self.font_title = tk.font.Font(family="Courier", size=14, weight="bold")
        self.font_btn   = tk.font.Font(family="Courier", size=11, weight="bold")
        self.font_small = tk.font.Font(family="Courier", size=9)
        self.font_big   = tk.font.Font(family="Courier", size=20, weight="bold")
        self.font_hex   = tk.font.Font(family="Courier", size=28, weight="bold")

        self.hex_value = tk.StringVar(value="0x00")
        self.bus_binary = tk.StringVar(value="00000000")
        self.bus_dec = tk.StringVar(value="0")
    ...
    def _build_ui(self):
        # ── Header ──
        header = tk.Frame(self.root, bg="#0f1218", pady=6)
        header.pack(fill="x")
        tk.Label(header, text="⬡ BUS DIGITAL CONTROLLER",
                 font=self.font_title, fg="#00e5ff", bg="#0f1218").pack(side="left", padx=16)
        modo = "SIMULACIÓN" if SIMULATION else "HARDWARE REAL"
        color_modo = "#ffaa00" if SIMULATION else "#00ff88"
        tk.Label(header, text=f"● {modo}",
                 font=self.font_small, fg=color_modo, bg="#0f1218").pack(side="right", padx=16)
        tk.Button(header, text="✕ SALIR", font=self.font_small,
                  bg="#ff3d71", fg="white", relief="flat", padx=8,
                  command=self._salir).pack(side="right", padx=8)

        # ── Contenido principal ──
        main = tk.Frame(self.root, bg="#0a0c10")
        main.pack(fill="both", expand=True, padx=10, pady=6)

        # Columna izquierda — LEDs
        left = tk.Frame(main, bg="#0a0c10")
        left.pack(side="left", fill="both", expand=True)

        tk.Label(left, text="CANALES DEL BUS",
                 font=self.font_small, fg="#4a5568", bg="#0a0c10").pack(anchor="w", pady=(0,4))
        ...
        for i, cfg in enumerate(LED_CONFIG):
            row = i // 2
            col = i % 2
            self._make_led_card(grid, i, cfg).grid(row=row, column=col, padx=4, pady=4, sticky="nsew")
            grid.columnconfigure(col, weight=1)

        # Columna derecha — Bus writer
        right = tk.Frame(main, bg="#0a0c10", width=220)
    ...
    def _make_led_card(self, parent, i, cfg):
        frame = tk.Frame(parent, bg="#0f1218", relief="flat",
        ...
        # Indicador LED
        indicator = tk.Label(top, text="●", font=tk.font.Font(size=16),
                             fg=cfg["color_dim"], bg="#0f1218")
        indicator.pack(side="left")

        tk.Label(top, text=cfg["nombre"],
                 font=self.font_btn, fg="#c8d6e5", bg="#0f1218").pack(side="left", padx=6)
        ...
        btn = tk.Button(frame, text="OFF", font=self.font_btn,
                        bg="#1a1e26", fg="#4a5568",
                        activebackground=cfg["color"], activeforeground="black",
                        relief="flat", pady=10,
        ...
        if cfg["pin"] in PWM_PINS:
            tk.Label(frame, text="BRILLO", font=self.font_small,
                     fg="#4a5568", bg="#0f1218").pack(anchor="w", padx=8)
        ...
        self.led_frames.append(card_data)
        return frame
    ...
    def _build_bus_panel(self, parent):
        ...
        # Display hex
        display = tk.Frame(parent, bg="#0f1218", highlightbackground="#1e2530", highlightthickness=1)
        display.pack(fill="x", pady=(0,8))
        tk.Label(display, textvariable=self.hex_value,
                 font=self.font_hex, fg="#00e5ff", bg="#0f1218").pack(pady=8)
        ...
        tk.Label(display, textvariable=self.bus_dec,
                 font=self.font_small, fg="#4a5568", bg="#0f1218").pack(pady=(0,8))
        ...
        for row_keys in keys:
            ...
            for k in row_keys:
                if k == '':
                    tk.Label(row, bg="#0a0c10", width=4).pack(side="left", expand=True, fill="x", padx=2)
                elif k == 'CLR':
                    tk.Button(row, text=k, font=self.font_btn,
                             bg="#1a1e26", fg="#ff3d71", relief="flat",
                             pady=12, command=self._hex_clear
                             ).pack(side="left", expand=True, fill="x", padx=2)
                elif k == '⌫':
                    tk.Button(row, text=k, font=self.font_btn,
                             bg="#1a1e26", fg="#ff3d71", relief="flat",
                             pady=12, command=self._hex_backspace
                             ).pack(side="left", expand=True, fill="x", padx=2)
                else:
                    tk.Button(row, text=k, font=self.font_btn,
                             bg="#1a1e26", fg="#c8d6e5", relief="flat",
                             pady=12, command=lambda c=k: self._hex_append(c)
                             ).pack(side="left", expand=True, fill="x", padx=2)

        # Botón enviar
        tk.Button(pad, text="⚡ ENVIAR AL BUS", font=self.font_btn,
                 bg="#00ff88", fg="#0a0c10", relief="flat", pady=14,
                 command=self._send_bus).pack(fill="x", pady=(8,4))

        # Presets
        tk.Label(parent, text="PRESETS",
                 font=self.font_small, fg="#4a5568", bg="#0a0c10").pack(anchor="w", pady=(8,2))
        presets_frame = tk.Frame(parent, bg="#0a0c10")
        presets_frame.pack(fill="x")
        for val in ["0x00", "0xFF", "0xAA", "0x55", "0x0F", "0xF0"]:
            tk.Button(presets_frame, text=val, font=self.font_small,
                     bg="#1a1e26", fg="#4a5568", relief="flat",
                     command=lambda v=val: self._set_hex(v)
                     ).pack(side="left", expand=True, fill="x", padx=1)

        # Controles globales
        tk.Label(parent, text="CONTROL GLOBAL",
                 font=self.font_small, fg="#4a5568", bg="#0a0c10").pack(anchor="w", pady=(12,2))
        tk.Button(parent, text="▶ TODOS ON", font=self.font_btn,
                 bg="#00e5ff", fg="#0a0c10", relief="flat", pady=10,
                 command=lambda: self._all_leds(True)).pack(fill="x", pady=2)
        tk.Button(parent, text="■ TODOS OFF", font=self.font_btn,
                 bg="#ff3d71", fg="white", relief="flat", pady=10,
                 command=lambda: self._all_leds(False)).pack(fill="x", pady=2)
    ...
    def _hex_backspace(self):
        ...
        self.hex_value.set(f"0x{current[:-1].zfill(1).upper() or '0'}")
